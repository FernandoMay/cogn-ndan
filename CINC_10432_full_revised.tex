...
\end{abstract}
\begin{keywords}
6G networks, semantic communication, edge intelligence, neuro-symbiotic networks, reconfigurable intelligent surfaces
\end{keywords}
\maketitle
...
\begin{equation}
\begin{aligned}
\text{Semantic Header} = \{&\text{IntentID},\\
&\text{CriticalityLevel},\ \text{CognitiveUrgency},\\
&\text{SemanticHash},\ \text{FeatureDim}\}.
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\text{CLI}(t) = \big(&\alpha \cdot P300\_\text{Amplitude}(t) + \beta \cdot \text{TBR}(t)\\
&+ \gamma \cdot \text{SpectralEntropy}(t)\big)/(\alpha + \beta + \gamma)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\text{Proof} = \{\text{Challenge}, \text{Response}\}:\\
|\text{FFT}(\text{Response}) - \text{UserSpectralTemplate}| < \theta_{max}
\end{aligned}
\end{equation}
...
\begin{tabular}{|p{3.0cm}|c|p{3.4cm}|}
\hline
\textbf{Evaluated property} & \textbf{Value} & \textbf{Notes} \\
\hline
Feature-space poisoning & 98.4\% & targeted $\epsilon$-bounded perturbations ($\epsilon \leq 0.05$) \\
Semantic-intent manipulation & 100\% & guaranteed by HMAC integrity over IntentLabel \\
EEG spoofing rejection & 96.1\% & ZKP over user spectral template, $\theta_{max}$ from enrollment \\
False-alarm rate & 0.6\% & benign packets mis-quarantined \\
...
\hline
\textbf{Metric} & \textbf{DeepSC} & \textbf{DRL-RIS} & \textbf{LrnSem} & \textbf{\COGN{}} \\
\hline
Mean latency (ms) & 28.1 & 35.4 & 30.2 & \textbf{23.0} \\
...
This paper introduced \COGN{}, a cross-layer protocol architecture that reorients 6G network design around semantic meaning and cognitive state awareness. By replacing bit-centric transmission with semantic routing and integrating real-time biological feedback loops, we have demonstrated a network architecture that is simultaneously faster (45\% latency reduction relative to gRPC), more efficient (40\% energy savings), and more robust under poor channel conditions.
...
The simulation code (NS-3 modules), experimental datasets, and manuscript source are being prepared for open-source release via the repository at \url{https://github.com/FernandoMay/cogn-ndan}. No human-participant data or animal experiments are included in this study; the EEG signals used are synthetic, procedurally generated waveforms calibrated against published normative ranges, not recordings from human subjects. Animal-experiment reporting is not applicable.
